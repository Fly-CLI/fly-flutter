...
extension StringExtension on String {
  ...
  String get toPascalCase => split('_').map((word) {
      if (word.isEmpty) return word;
      return word[0].toUpperCase() + word.substring(1).toLowerCase();
    }).join();
  ...
  bool get isValidPhone => RegExp(r'^\+?[1-9]\d{1,14}$').hasMatch(replaceAll(RegExp(r'[\s\-\(\)]'), ''));
...
extension MapExtension<K, V> on Map<K, V> {
  ...
  Map<K, V> get removeNullValues => Map.fromEntries(
...
extension BuildContextExtension on BuildContext {
  ...
  Future<T?> navigateTo<T>(Widget screen) => Navigator.of(this).push<T>(
      MaterialPageRoute(builder: (context) => screen),
    );
  
  /// Navigate to a new screen and replace current
  Future<T?> navigateToReplacement<T>(Widget screen) => Navigator.of(this).pushReplacement<T, dynamic>(
      MaterialPageRoute(builder: (context) => screen),
    );
