...
/// and lifecycle management using Riverpod Notifier.
abstract class BaseViewModel<T> extends Notifier<ViewState<T>> {
  @override
  ViewState<T> build() => ViewState<T>.idle();
  
  /// Get the current state
  /// 
  /// This is a public getter that can be accessed from outside the ViewModel.
  ViewState<T> get currentState => state;
  ...
  T? getData() => state.data;
  
  /// Get the current error if the state is error
  Object? getError() => state.error;
...
class BaseViewModelProvider extends _$BaseViewModelProvider {
  @override
  ViewState<Object?> build() => const ViewState<Object?>.idle();
}
