...
import 'package:fly_cli/src/core/command/foundation/domain/command_lifecycle.dart';
...
abstract class FlyCommand extends Command<int> implements CommandLifecycle {
  ...
  Future<int> run() async {
    // Create a new immutable execution context with actual args at the very start
    // This must be the FIRST thing to ensure ProgressFactory and other early
    // code can access the correct flags (e.g., --plan, --format, --quiet)
    // CommandRunner sets argResults before calling run(), so we can use it here
    ...
    final executionContext = CommandExecutionContext(
      ...
      progressTracker: ProgressFactory.create(executionCommandContext),
    );
    executionCommandContext.setData('execution_context', executionContext);
    ...
    try {
      ...
      await onBeforeExecute(executionCommandContext);
      ...
      final result = await _runMiddlewarePipeline(executionCommandContext);
      ...
      await onAfterExecute(executionCommandContext, enhancedResult);
      ...
      executionContext.setPhase(ExecutionPhase.error);
      await onError(executionCommandContext, e, stackTrace);
  ...
  StreamSubscription<ProcessSignal> _setupCancellationHandler(
    ...
    final subscription = ProcessSignal.sigint.watch().listen((signal) {
      ...
      // Note: Use registration context for this check since execution context
      // may not be available during signal handling
  ...
  Future<ValidationResult> _runValidators(CommandContext executionContext) async {
    ...
    // Use argResults which contains actual parsed args
