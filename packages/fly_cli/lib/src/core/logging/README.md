...
`LoggingConfig.fromEnvironment(isProd: ...)` populates defaults: dev → human/debug; prod →
json/info.
...
- `LoggingContext` includes environment, version, command, args, workspace, feature flags, and
  custom extras.
...
- Single kill switch: set `FLY_LOG_SHIP=1` to allow remote appenders to activate; `0` (default)
  disables them.
- Provider activation:
...
- The root logger is created in `command_runner.dart` using `LoggerFactory` and is injected via the
  service container.
...
- Use `LoggerAdapter` to mirror messages to both `mason_logger` and the structured logger during
  transition.
...
- Q: Does this send telemetry by default?
    - A: No. Remote shipping is opt-in and gated by `FLY_LOG_SHIP=1` plus provider-specific env
      vars.
- Q: Can I self-host?
    - A: Yes. Use `HttpAppender` with any endpoint (ELK/OpenSearch/OpenObserve/Loki-compatible
      gateways).
