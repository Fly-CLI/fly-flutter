...
class FeatureVariableBuilder implements GenerationVariableBuilder {
  ...
  ValidationResult validate(Map<String, dynamic> rawVars) {
    ...
    final name = rawVars['name'] as String?;
    if (name == null || name.isEmpty) {
      errors.add('Feature name is required');
    } else {
      final nameValidation = NameValidationRule.validateScreenName(name);
      if (!nameValidation.isValid) {
        errors.addAll(nameValidation.errors);
      }
    }

    final feature = rawVars['feature'] as String?;
    if (feature != null && feature.isNotEmpty) {
      final featureValidation = NameValidationRule.validateFeatureName(feature);
...
class ServiceVariableBuilder implements GenerationVariableBuilder {
  ...
  ValidationResult validate(Map<String, dynamic> rawVars) {
    ...
    if (name == null || name.isEmpty) {
      ...
      final nameValidation = NameValidationRule.validateServiceName(name);
      if (!nameValidation.isValid) {
        errors.addAll(nameValidation.errors);
      }
    }
...
class ProjectVariableBuilder implements GenerationVariableBuilder {
  ...
  ValidationResult validate(Map<String, dynamic> rawVars) {
    ...
    if (name == null || name.isEmpty) {
      ...
      final nameValidation = NameValidationRule.validateProjectName(name);
      if (!nameValidation.isValid) {
        errors.addAll(nameValidation.errors);
      }
    }

    final organization = rawVars['organization'] as String?;
    if (organization != null && organization.isNotEmpty) {
      // Basic validation for organization format
      if (!organization.contains('.')) {
        errors.add('Organization must be in reverse domain format (e.g., com.example)');
      }
    }
