...
abstract class CommandMiddleware {
  ...
  Future<CommandResult?> handle(
      CommandContext context, Future<CommandResult?> Function() next);
...
class MiddlewarePipeline {
  ...
  Future<CommandResult?> execute(CommandContext context,
      Future<CommandResult?> Function() commandExecute) async {
...
class LoggingMiddleware implements CommandMiddleware {
  ...
  @override
  Future<CommandResult?> handle(
      CommandContext context, Future<CommandResult?> Function() next) async {
    final stopwatch = Stopwatch()..start();
    context.logger.detail(
        'Executing command: ${context.argResults.command?.name ?? 'root'} with args: ${context.argResults.arguments}');
    ...
    try {
      ...
      if (result != null) {
        context.logger.detail(
            'Command ${context.argResults.command?.name ?? 'root'} completed in ${stopwatch.elapsedMilliseconds}ms with status: ${result.success ? 'SUCCESS' : 'FAILURE'}');
      }
      return result;
    } catch (e, st) {
      stopwatch.stop();
      context.logger.err(
          'Command ${context.argResults.command?.name ?? 'root'} failed in ${stopwatch.elapsedMilliseconds}ms with error: $e');
  ...
  Future<CommandResult?> handle(
    ...
    try {
      ...
      if (result != null) {
        ...
        context.setData(
            'command_name', context.argResults.command?.name ?? 'root');
      }
    ...
    if (planMode || hasPlanFlag) {
      ...
      return CommandResult.success(
        ...
        nextSteps: [
          NextStep(
            command:
                'fly ${context.argResults.command?.name ?? 'command'} [args]',
