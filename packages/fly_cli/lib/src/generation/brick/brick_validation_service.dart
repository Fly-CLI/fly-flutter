...
class BrickValidationService {
  ...
  Future<BrickValidationResult> validate(Brick brick) async {
    final errors = <String>[];
    final warnings = <String>[];

    try {
      // Basic validation
      if (brick.name.isEmpty) {
      ...
      if (!brick.isValid) {
        errors.addAll(brick.validationErrors);
      }
  ...
  Future<BrickValidationResult> _validateBrickStructure(Brick brick) async {
    final errors = <String>[];
    final warnings = <String>[];
  ...
  Future<void> _validateScreenBrickStructure(
    ...
    final screenFile = File(
      path.join(brickContentDir.path, '{{screen_name}}_screen.dart'),
    );
  ...
  Future<void> _validateServiceBrickStructure(
    ...
    final serviceFile = File(
      path.join(brickContentDir.path, '{{service_name}}_service.dart'),
    );
  ...
  Future<void> _validateWidgetBrickStructure(
    ...
    final widgetFile = File(
      path.join(brickContentDir.path, '{{widget_name}}_widget.dart'),
    );
  ...
  Future<BrickValidationResult> _validateBrickCompatibility(Brick brick) async {
    final errors = <String>[];
    final warnings = <String>[];
