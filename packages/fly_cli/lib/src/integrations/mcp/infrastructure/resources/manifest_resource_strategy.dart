  ...
  Map<String, Object?> read(Map<String, Object?> params) {
    ...
    if (uri == null || uri.isEmpty) {
      throw ResourceError.invalidUri(
        resourceUri: uri ?? '',
        expectedFormat: 'manifest://<filename>',
        hints: [
          'URI cannot be null or empty',
          'Manifest URIs must start with manifest://',
          'Example: manifest://pubspec.yaml',
        ],
      );
    }
    ...
    if (!_allowedFiles.contains(fileName)) {
      throw ResourceError.invalidManifestFile(
        fileName: fileName,
        allowedFiles: _allowedFiles,
      );
    }
    ...
    if (!file.existsSync()) {
      throw ResourceError.notFound(
        ...
        resourceUri: uri,
        suggestions: [
          'The manifest file may not exist yet',
          'Create the file if needed: $fileName',
          'Use resources/list to see available manifest files',
        ],
      );
    }
