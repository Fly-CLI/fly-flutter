...
class LogsRunResourceStrategy extends ResourceStrategy {
  ...
  @override
  String get description => 'Execution logs from process runs';
  ...
  Map<String, Object?> list(Map<String, Object?> params) {
    ...
    // Extract process ID prefix from URI if provided
    // listLogs expects prefix to match process ID, not full URI
    String? prefix;
    if (uriPrefixParam != null && uriPrefixParam.startsWith('logs://run/')) {
      // Extract the process ID part after 'logs://run/'
      final processIdPart = uriPrefixParam.replaceFirst('logs://run/', '');
      prefix = processIdPart.isEmpty ? null : processIdPart;
    } else {
      // No URI prefix means list all run logs
      prefix = null;
    }
  ...
  Map<String, Object?> read(Map<String, Object?> params) {
    ...
    if (uri == null || !uri.startsWith('logs://run/')) {
