...
class JsonRpcConnection {
  ...
  Future<void> start({
    ...
    await for (final frame
        in transport.framedInput(maxMessageBytes: maxMessageBytes)) {
      ...
      if (decoded['method'] != null && decoded['id'] != null) {
        ...
        try {
          ...
          final error = JsonRpcError(
              code: -32603, message: 'Internal error', data: e.toString());
        ...
        if (method == '\$/cancelRequest') {
          ...
          final cancelReq = JsonRpcRequest.fromJson({
            ...decoded,
            'id':
                'cancel_${DateTime.now().microsecondsSinceEpoch}', // Generate temp ID
          });
          try {
