...
extension ApiResponseExtension<T> on ApiResponse<T> {
  ...
  ApiResponse<R> mapData<R>(R Function(T data) mapper) {
    return switch (this) {
      ApiSuccess<T>(data: final data, statusCode: final statusCode, headers: final headers, message: final message) => 
        ApiResponse.success(mapper(data), statusCode: statusCode, headers: headers, message: message),
      ApiFailure<T>(error: final error, statusCode: final statusCode, headers: final headers) => 
        ApiResponse.failure(error, statusCode: statusCode, headers: headers),
    };
  }
  
  /// Handle the response with different functions for success and failure
  R when<R>({
    required R Function(T data, int? statusCode, Map<String, dynamic>? headers, String? message) success,
    required R Function(ApiError error, int? statusCode, Map<String, dynamic>? headers) failure,
  }) {
    return switch (this) {
      ApiSuccess<T>(data: final data, statusCode: final statusCode, headers: final headers, message: final message) => 
  ...
  R maybeWhen<R>({
    R Function(T data, int? statusCode, Map<String, dynamic>? headers, String? message)? success,
    R Function(ApiError error, int? statusCode, Map<String, dynamic>? headers)? failure,
  ...
  T getOrThrow() {
    return switch (this) {
      ApiSuccess<T>(data: final data) => data,
      ApiFailure<T>(error: final error) => throw Exception('API request failed: $error'),
    };
  }
