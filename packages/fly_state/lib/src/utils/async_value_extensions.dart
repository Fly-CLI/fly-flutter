...
extension FlyAsyncValueListExtension<T> on AsyncValue<List<T>> {
  ...
  AsyncValue<List<T>> take(int count) =>
      mapData((data) => data.take(count).toList());

  /// Skip the first n elements
  AsyncValue<List<T>> skip(int count) =>
      mapData((data) => data.skip(count).toList());

  /// Get a sublist
  AsyncValue<List<T>> sublist(int start, [int? end]) =>
      mapData((data) => data.sublist(start, end));
}
...
extension FlyAsyncValueMapExtension<K, V> on AsyncValue<Map<K, V>> {
  ...
  /// Get all entries
  AsyncValue<Iterable<MapEntry<K, V>>> get entries =>
      mapData((data) => data.entries);

  /// Map the map values
  AsyncValue<Map<K, R>> mapValues<R>(R Function(V value) mapper) =>
      mapData((data) => data.map((key, value) => MapEntry(key, mapper(value))));

  /// Map the map keys
  AsyncValue<Map<R, V>> mapKeys<R>(R Function(K key) mapper) =>
      mapData((data) => data.map((key, value) => MapEntry(mapper(key), value)));
  ...
  AsyncValue<Map<K, V>> removeNullValues() => mapData((data) =>
      Map<K, V>.from(data)..removeWhere((key, value) => value == null));
}
