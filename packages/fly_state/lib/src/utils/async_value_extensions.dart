...
extension FlyAsyncValueExtension<T> on AsyncValue<T> {
  ...
  /// Whether the value has an error (renamed to avoid conflicts)
  bool get hasAnError => when(
    data: (_) => false,
    loading: () => false,
    error: (_, __) => true,
  );
  ...
  AsyncValue<R> mapData<R>(R Function(T data) mapper) {
    return when(
      ...
      error: (error, stackTrace) => AsyncValue.error(error, stackTrace ?? StackTrace.current),
    );
  }
  
  /// Map the data to a new AsyncValue
  AsyncValue<R> mapAsync<R>(AsyncValue<R> Function(T data) mapper) {
    return when(
  ...
  T getOrThrow() {
    return when(
      data: (data) => data,
      loading: () => throw StateError('AsyncValue is still loading'),
      error: (error, stackTrace) => throw error,
    );
  }
  
  /// Handle the value with different functions (renamed to avoid conflicts)
  R handle<R>({
...
extension FlyAsyncValueListExtension<T> on AsyncValue<List<T>> {
  ...
  /// Filter the list
  AsyncValue<List<T>> where(bool Function(T element) test) {
    return mapData((data) => data.where(test).toList());
  }
  
  /// Map the list elements
  AsyncValue<List<R>> mapList<R>(R Function(T element) mapper) {
    return mapData((data) => data.map(mapper).toList());
  }
  
  /// Sort the list
  AsyncValue<List<T>> sorted([int Function(T a, T b)? compare]) {
    return mapData((data) {
  ...
  /// Reverse the list
  AsyncValue<List<T>> reversed() {
    return mapData((data) => data.reversed.toList());
  }
  
  /// Take the first n elements
  AsyncValue<List<T>> take(int count) {
    return mapData((data) => data.take(count).toList());
  }
  
  /// Skip the first n elements
  AsyncValue<List<T>> skip(int count) {
    return mapData((data) => data.skip(count).toList());
  }
  
  /// Get a sublist
  AsyncValue<List<T>> sublist(int start, [int? end]) {
    return mapData((data) => data.sublist(start, end));
  }
}
...
extension FlyAsyncValueMapExtension<K, V> on AsyncValue<Map<K, V>> {
  ...
  /// Map the map values
  AsyncValue<Map<K, R>> mapValues<R>(R Function(V value) mapper) {
    return mapData((data) => data.map((key, value) => MapEntry(key, mapper(value))));
  }
  
  /// Map the map keys
  AsyncValue<Map<R, V>> mapKeys<R>(R Function(K key) mapper) {
    return mapData((data) => data.map((key, value) => MapEntry(mapper(key), value)));
  }
  
  /// Filter the map
  AsyncValue<Map<K, V>> where(bool Function(K key, V value) test) {
    return mapData((data) => data.entries.where((entry) => test(entry.key, entry.value)).fold<Map<K, V>>({}, (map, entry) => {...map, entry.key: entry.value}));
  }
  
  /// Remove entries where the value is null
  AsyncValue<Map<K, V>> removeNullValues() {
    return mapData((data) => Map<K, V>.from(data)..removeWhere((key, value) => value == null));
  }
}
