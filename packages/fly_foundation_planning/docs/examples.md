...
final result = planner.planFromRequest(request);
```
...
final result = planner.planFromRequest(PlanningRequest(
...
try {
  ...
  workflow.validateBrickIds(registry);  // Throws PlanningException
} on PlanningException catch (e) {
...
```dart
test('plans foundation project correctly', () {
  final planner = FoundationPlanner();
...
```dart
test('full planning and execution pipeline', () async {
  final planner = FoundationPlanner();
