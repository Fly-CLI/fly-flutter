...
This document explains how the planning system transforms user input into an executable plan.

## Overview

The planning process converts raw user input (flags, manifest, prompts) into an ordered list of `BrickInvocation` objects, each ready for execution.
...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│ PlanningRequest │  Normalized request with workflow ID
└────────┬────────┘
         │
         ▼
┌─────────────────┐
...
- Creates normalized `PlanningRequest`
...
The planner returns a `PlanningResult`:
...
All errors are `PlanningException` with descriptive messages.
...
The planning process is fully testable:

```dart
test('plans foundation project with features', () {
