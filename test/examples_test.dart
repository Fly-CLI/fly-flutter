...
void main() {
  group('Example Projects', () {
    test('minimal_example has correct structure', () {
      ...
      expect(
        File(path.join(projectDir.path, 'pubspec.yaml')).existsSync(),
        isTrue,
      );
      expect(
      ...
      expect(
        File(
          path.join(projectDir.path, 'test', 'widget_test.dart'),
        ).existsSync(),
        isTrue,
      );
      expect(
        File(path.join(projectDir.path, 'README.md')).existsSync(),
        isTrue,
      );
    });
    ...
    test('riverpod_example has correct structure', () {
      ...
      expect(
        Directory(path.join(projectDir.path, 'lib', 'features')).existsSync(),
        isTrue,
      );
      expect(
        Directory(path.join(projectDir.path, 'lib', 'core')).existsSync(),
        isTrue,
      );
      expect(
        Directory(
          path.join(projectDir.path, 'lib', 'features', 'home'),
        ).existsSync(),
        isTrue,
      );
      expect(
        Directory(
          path.join(projectDir.path, 'lib', 'features', 'profile'),
        ).existsSync(),
        isTrue,
      );
    });
    ...
    test('minimal_example pubspec.yaml is valid', () {
      ...
      expect(
        content.contains(
          'description: A minimal Flutter app created with Fly CLI',
        ),
        isTrue,
      );
    ...
    test('riverpod_example pubspec.yaml is valid', () {
      ...
      expect(
        content.contains(
          'description: A production-ready Riverpod Flutter app created with Fly CLI',
        ),
        isTrue,
      );
    ...
    test('riverpod_example has proper feature structure', () {
      final homeScreen = File(
        'examples/riverpod_example/lib/features/home/presentation/home_screen.dart',
      );
      expect(homeScreen.existsSync(), isTrue);

      final homeProvider = File(
        'examples/riverpod_example/lib/features/home/providers/home_provider.dart',
      );
      expect(homeProvider.existsSync(), isTrue);

      final profileScreen = File(
        'examples/riverpod_example/lib/features/profile/presentation/profile_screen.dart',
      );
      expect(profileScreen.existsSync(), isTrue);

      final profileProvider = File(
        'examples/riverpod_example/lib/features/profile/providers/profile_provider.dart',
      );
      expect(profileProvider.existsSync(), isTrue);
    });

    test('examples have proper test files', () {
      final minimalTest = File(
        'examples/minimal_example/test/widget_test.dart',
      );
      expect(minimalTest.existsSync(), isTrue);

      final riverpodTest = File(
        'examples/riverpod_example/test/widget_test.dart',
      );
